...
# Compute the mortality outcome: positive sum -> survived, 0 sum -> died
# Step 1: Get the lengths of each trajectory manually
traj_lengths = test_data.groupby('traj').size().values  # size() gives counts

# Step 2: Now get the mortality
test_mortality = torch.Tensor(test_data.groupby('traj')['r:reward'].sum().values)  # mortality sum

# Step 3: Filter: trajectories with length > 1
valid_indices = traj_lengths > 1  # boolean mask

# Step 4: Apply filtering
test_mortality = test_mortality[valid_indices]  # only mortality for valid trajectories

# Step 5: Expand dims and repeat
test_mortality = test_mortality.unsqueeze(1).unsqueeze(2)  # [N,1,1]
test_mortality = test_mortality.repeat(1, 20, 1)           # [N,20,1]
