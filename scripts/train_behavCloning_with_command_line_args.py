...
def run(BC_network, train_dataloader, val_dataloader, num_epochs, storage_dir, loss_func, device):
    ...
    for i_epoch in range(num_epochs):
        ...
        if i_epoch % eval_frequency == 0:
            ...
            # Early stopping logic for BC validation loss
            if len(validation_losses) > 1:
                best_so_far = min(validation_losses[:-1])
                if mean_val_loss > 1.01 * best_so_far:
                    print(f"Early stopping triggered: validation loss {mean_val_loss:.4f} grew by more than 3% from best {best_so_far:.4f}")
                    break  # exits the training loop
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-demographics', dest='dem_context', default=False, action='store_false')
    ...
    common_params.update({k: v for k, v in vars(args).items() if k in common_params})
    print("storage_folder is : ", behavCloning_params["storage_folder"])
    os.makedirs(behavCloning_params["storage_folder"], exist_ok=True)
    ...
    print("whether include dem_context : ", common_params["dem_context"])
    ...
    train_buffer = ReplayBuffer(input_dim, behavCloning_params["batch_size"], 200000, device, encoded_state=False, obs_state_dim=input_dim)
    ...
    val_buffer = ReplayBuffer(input_dim, behavCloning_params["batch_size"], 50000, device, encoded_state=False, obs_state_dim=input_dim)
